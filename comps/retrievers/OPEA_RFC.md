...
## Abstract:
We found some problems in GenAIComps retrieve and dataprep implementations. The problems are:
1. There are duplicated and unaligned code in retreival and data-prep micro-service. The implementation adapts to integrate different database (Redis, Milus, Pgvector, etc) and adapt to different frameworks (langchain, llama_index, haystack).
2. Inconsistent implementation of software stack layer: most of the mirocs-service retrieval instances are implemented through database layer while Qdrant is implemented thought retrieval layer.

Scalable and Seamless are two advantanges of OPEA. These two problems hurt the scalable and seamless of OPEA.
We propose to define clear software stack layer and clear interface for database to solve the problems.

The RFC is organzied as following:
Section 1 is problem statement, then we analysis the reaons that lead to these problems in sectoins 2. Finally, we give our proposas to solve the current problems, and implement a proto-type to show our proposal to in section 3.
